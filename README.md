...
- 설치 파일: [`dist/moa-calendar-0.1.1-debug.apk`](dist/moa-calendar-0.1.1-debug.apk) (약 12MB).
...
네이버 공식 도움말은 **Android CalDAV를 지원하지 않는다**고 명시합니다. 이 앱의 직접 연결은 표준 HTTPS CalDAV로 호환성을 확인하는 실험적 기능이며, 네이버 실계정 연결 성공을 보장하지 않습니다. iOS/macOS 전용 클라이언트인 것처럼 위장하지 않습니다.
...
- 실계정 인증/클라우드 동기화는 개발 환경에서 검증하지 않았습니다. Google 기기 동기화와 네이버의 서버 정책에 의존합니다.
...
v0.1.1 검증 결과는 [`docs/VALIDATION-0.1.1.md`](docs/VALIDATION-0.1.1.md), 새 화면은 `docs/screenshots/v0.1.1/`에 있습니다. 이전 버전의 검증 기록은 `docs/VALIDATION.md`에 보존합니다. 디자인 규칙은 [`DESIGN.md`](DESIGN.md)에 있습니다.
