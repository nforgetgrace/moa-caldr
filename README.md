...
- 설치 파일: [`dist/moa-calendar-0.1.8-debug.apk`](dist/moa-calendar-0.1.8-debug.apk) (약 12MB).
...
- 네이버 재동기화는 서버의 ETag와 저장된 버전을 비교해 새로 생기거나 변경된 일정·할 일 본문을 가져옵니다. 독립된 조회는 최대 3개씩 병행하고 연결을 재사용합니다. 서버가 해당 조회를 지원하지 않으면 기존 전체 조회로 복귀합니다. 기존 조회 기간과 삭제 확인은 유지합니다.
...
v0.1.8 검증 결과와 통제된 환경의 성능 측정은 [`docs/VALIDATION-0.1.8.md`](docs/VALIDATION-0.1.8.md), 화면은 `docs/screenshots/v0.1.8/`에 있습니다. 이전 검증 기록은 각 버전의 `docs/VALIDATION-*.md`에 보존합니다. 디자인 규칙은 [`DESIGN.md`](DESIGN.md)에 있습니다.
