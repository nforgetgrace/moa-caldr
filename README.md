...
- 설치 파일: [`dist/moa-calendar-0.1.12-debug.apk`](dist/moa-calendar-0.1.12-debug.apk) (약 12MB).
...
- 위젯 새로고침을 누르면 위젯 안에 스피너를 표시하고, 동기화가 끝나면 상태줄에 `최근 동기화 M/d HH:mm`을 작게 표시합니다.
- 월 위젯 헤더는 `‹ 9월 ›`를 가운데에 두고 오른쪽에 Gook's 표시와 새로고침을 둡니다.
...
- 월/주 보기, 오늘 이동, 날짜별 일정, 다가오는 일정, 제목·장소·메모 검색. 앱 날짜 칸에서는 `+N`으로 숨기지 않고 모든 일정 행을 표시하며, 하단 ‘일정 보기’ 버튼으로 전체 제목·시간·계정 목록을 확인합니다.
- 기본 표시색은 네이버 일정·할 일 연보라, Google 일정 초록이며 기기 캘린더는 원래 색상입니다. 캘린더마다 설정에서 원하는 표시색으로 바꿀 수 있습니다. 표시 여부 필터는 위젯에도 적용됩니다.
...
- 월간 라이트 위젯은 날짜마다 최대 3개 일정 제목과 별도의 `+N` 줄을 표시합니다(총 5개 → 3개 제목 + `+2`). 여백을 줄이고 제목 글자를 8sp에서 10sp로 키웠습니다. 작은 위젯이나 글자 확대 상태에서는 공간에 맞춰 제목 수를 줄이며 초과 개수는 정확히 표시합니다.
...
- 네이버 재동기화는 유효한 강한 ETag가 있으면 저장된 버전과 비교해 변경된 본문을 가져옵니다. 버전 정보가 없거나 지원 형식이 아니어도 필요한 본문을 최대 50개씩 묶어 요청합니다. 독립된 조회는 최대 3개씩 병행하고 연결을 재사용합니다. 서버가 묶음 조회를 지원하지 않으면 기존 전체 조회와 GET으로 복귀합니다. 기존 조회 기간과 삭제 확인은 유지합니다.
- 재부팅/앱 업데이트/시간대·날짜 변경 시 작업을 복구하고 위젯을 갱신합니다.
- 캐시를 통한 네이버 오프라인 조회. 서버 동기화가 진행 중이어도 저장된 일정을 즉시 읽으며, 검증한 일정·할 일·상태를 한 번에 반영합니다. 실패 시 캐시 유지와 오류 표시.
- 기기 캘린더 조회 실패 시 마지막 정상 목록과 일정을 함께 유지합니다. 정상 빈 결과와 권한 회수는 실제로 반영합니다. 백그라운드 갱신이 이전 조회 기간을 축소하지 않도록 캐시 범위를 유지합니다.
- 여러 날 일정은 앱·월간 위젯에서 하나의 가로 막대로 연결합니다. 주가 바뀌면 이어짐 기호와 함께 다음 주로 계속 표시합니다. 종료 시각이 자정이면 다음 날짜를 잘못 포함하지 않습니다.
- Google·네이버 일정 저장은 긴 네이버 백그라운드 조회를 기다리지 않습니다. 늦게 도착한 이전 조회 결과나 오류가 저장·삭제·계정 변경을 되돌리지 않도록 검증합니다.
- 최근 실행 앱에 모아가 남아 있어도 위젯에서 누른 날짜를 기존 앱에서 바로 표시합니다. 막대 중간을 눌러도 실제 누른 날짜를 선택합니다.
- 상단 스피너는 수동·자동 동기화가 진행되는 동안 표시하고 완료·실패·취소 뒤 사라집니다. 다음 동기화가 시작되면 다시 표시합니다. 로그인 뒤 네이버 백그라운드 조회와 Android가 알려주는 선택한 Google 계정의 동기화 상태도 반영하며, 기존 일정은 계속 표시합니다.
- Google 동기화 알림을 놓친 경우에도 복구하도록 앱이 활성화된 동안 2초마다 기기 동기화 상태를 확인합니다. 이 확인은 상태 조회만 수행하며 서버 동기화를 새로 요청하지 않습니다.
...
- 반복 일정은 반복 없음·매일·매주·매월·매년으로 생성하거나 전체 반복을 수정할 수 있습니다. 반복 수정 화면에는 첫 일정의 날짜를 표시합니다. 개별 변경·예외가 있는 반복, 개별 회차 수정과 반복 삭제는 원본 앱에서 수행합니다. `RANGE=THISANDFUTURE`는 잘못 표시하는 대신 지원 제한 오류를 표시합니다.
...
- 네이버 캐시는 성공한 조회 범위를 누적해 유지하며, 그 전체 범위를 다시 조회해 변경·삭제를 확인합니다. 오프라인에서 한 번도 조회하지 않은 기간의 일정은 보장하지 않습니다.
...
v0.1.12 위젯 새로고침·동기화 시각 검증 결과는 [`docs/VALIDATION-0.1.12.md`](docs/VALIDATION-0.1.12.md), 테스트 화면은 `docs/screenshots/v0.1.12/`에 있습니다. v0.1.11 위젯 월 이동 검증 결과는 [`docs/VALIDATION-0.1.11.md`](docs/VALIDATION-0.1.11.md)에 있습니다. v0.1.10 반복·색상 검증 결과는 [`docs/VALIDATION-0.1.10.md`](docs/VALIDATION-0.1.10.md)에 있습니다. 갤럭시는 v0.1.9 설치 후 다른 세션에 넘겨 추가 조작·설치하지 않았습니다. 당시 실기기 검증은 [`docs/VALIDATION-0.1.9.md`](docs/VALIDATION-0.1.9.md)에 있습니다. 이전 검증 기록은 각 버전의 `docs/VALIDATION-*.md`에 보존합니다. 디자인 규칙은 [`DESIGN.md`](DESIGN.md)에 있습니다.
