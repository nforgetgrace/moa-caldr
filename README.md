...
- 설치 파일: [`dist/moa-calendar-0.1.3-debug.apk`](dist/moa-calendar-0.1.3-debug.apk) (약 12MB).
...
- 월/주 보기, 오늘 이동, 날짜별 일정, 다가오는 일정, 제목·장소·메모 검색. 날짜 칸에 짧은 일정 제목과 `+N`을 표시하며, 하단 ‘일정 보기’ 버튼으로 전체 제목·시간·계정 목록을 확인합니다.
...
- 최근 실행 앱에 모아가 남아 있어도 위젯에서 누른 날짜를 기존 앱에서 바로 표시합니다.
...
v0.1.3 검증 결과는 [`docs/VALIDATION-0.1.3.md`](docs/VALIDATION-0.1.3.md), 화면은 `docs/screenshots/v0.1.3/`에 있습니다. 이전 검증 기록은 각 버전의 `docs/VALIDATION-*.md`에 보존합니다. 디자인 규칙은 [`DESIGN.md`](DESIGN.md)에 있습니다.
